  ...
  async burnAsset(
    ...
    opts: {
      ...
      serviceEntries?: Array<{
        body: string // '<bodyHex>' // from step 2
        flags: number // 0 // no compression, no encryption
        instruction: string // 'K' // Bridgeless uses "K" to mark payload carrier
        security: string // '<pubkey>' // Bridgeless operator or your wallet’s pubkey
        service_id: string // 'C' // another Bridgeless internal tag
      }>
    }
  ): Promise<string> {
    const params = {
